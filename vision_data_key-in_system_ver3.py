...
def load_analysis_data():
    ...
    for ws in target_sheets:
        ...
        # 💡 스마트 컬럼 맵핑 (이름이 조금씩 달라도 완벽하게 동일한 구조로 강제 통합)
        rename_dict = {}
...
if st.session_state.current_page == "analysis":
    ...
    if df.empty or '모델명(MI)' not in df.columns: 
        ...
        # 💡 강제 필터링: 테스트용으로 무조건 최근 150건 가져오기 (날짜 조건 삭제)
        ...
        with st.container(border=True):
            ...
            if not selected_models:
                ...
                if model_df.empty:
                    ...
                    # 💡 X축을 실제 시간으로 매핑
                    # 만약 시간이 올바르지 않다면 순서대로라도 뿌리기 위해 방어
                    ...
                    with st.container(border=True):
                        ...
                        for idx, mod in enumerate(selected_models):
                            ...
                            # 💡 꺾은선 점 위에 LOT 번호가 표시되게 mode에 text 추가
